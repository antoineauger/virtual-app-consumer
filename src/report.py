...
class Report(object):
    def __init__(self, application_id, timestamps, battery_level, provenance):
        self.metrics = dict()
...
class RDorInformationReport(Report):
    def __init__(self, observation, config, battery_level):
        super().__init__(config['application_id'], observation['timestamps'], battery_level, observation['producer'])


class KnowledgeReport(Report):
    def __init__(self, observation, config, battery_level):
        extracted_fields = ObsUtils.extract_fields_from_jsonld(observation)
        super().__init__(config['application_id'], extracted_fields['timestamps'], battery_level, extracted_fields['producer'])
